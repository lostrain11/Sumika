...
- 模型选择、原生计划状态、回合和压缩边界作为恢复观察保留；助手正文和工具执行历史直接复用 DSH 原生日志，不再重复采集。既有 message/tool 记录保持可读，不迁移或删除。工具结束和回合结束不等于任务完成。不保存 reasoning、请求头凭据或原始工具输入输出。没有修改代码、diff 或成果正文的钩子。
...
P4 精简：没有新增有效记录时只推进内存游标，不启动存储子进程、不重写交接文件；需要注入而本次没有新记录时重新读取项目状态，避免跨会话缓存过期。现有 SQLite 数据格式及严格校验保留；替换为其他插件须先验证迁移与来源边界，不同时启用两套自动规划/记忆接线。
