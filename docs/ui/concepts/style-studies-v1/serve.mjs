...
const exact = {
  ...
  '/shared/lucide.min.js':resolve(directory,'../warm-home-v1/vendor/lucide.min.js'),
};
  ...
  const server=createServer(async(request,response)=>{
    ...
    try {
      ...
      if(!target) {
        ...
        if(!target.startsWith(base+sep)||relative.includes('..')||relative.includes('\\')||relative.includes(':')){response.writeHead(403).end();return;}
      }
...
if(process.argv[1] && resolve(process.argv[1])===fileURLToPath(import.meta.url)) {
  let preview;
  try {preview=await startPreview(Number(process.argv[2]||8877));}
  catch(error){if(error.code!=='EADDRINUSE')throw error;preview=await startPreview();}
  console.log(`Sumika style studies: ${preview.url}`);
