...
| [Open-LLM-VTuber](https://github.com/Open-LLM-VTuber/Open-LLM-VTuber) and its [web client](https://github.com/Open-LLM-VTuber/Open-LLM-VTuber-Web) | companion-shell reference for the 2026-09 scene-first UI reset: pet/full dual mode, settings drawer + tabs structure, per-mode window management ideas | MIT; interaction/layout patterns studied locally (`D:\Code\ui-refs` shallow clone), no source file, style or asset copied; every visible surface is re-expressed with Sumika design tokens |
| [Amica](https://github.com/semperai/amica) | fullscreen immersive layering model (canvas z-0, menu z-10, input z-20, modal z-40), mutually exclusive panel toggles, runtime background color/image application | MIT; patterns re-implemented in vanilla JS/CSS, no code copied |
| [ChatVRM](https://github.com/pixiv/ChatVRM) | chat bubble structure, scroll anchoring on the last message, fullscreen glass overlay settings | MIT; patterns re-implemented, no code copied |
...
| [Qwen3](https://huggingface.co/Qwen/Qwen3-1.7B) `qwen3:1.7b` Ollama model | user-selected isolated fast-test model; not the production default and not bundled | Qwen license terms apply to downloaded weights; Sumika does not bundle or redistribute the model; use is limited to the user's local test cache |
| [Tencent BrowserSkill](https://github.com/Tencent/BrowserSkill/tree/a004291848e8641400b973b8d612b4c4b74cdc90) CLI, `packages/dsh-plugin-browserskill` and the official `browser-skill-extension-v0.1.7-chrome.zip` release artifact | optional BrowserSkill health/session bridge and DSH plugin boundary | MIT; source reference `a004291848e8641400b973b8d612b4c4b74cdc90`; fixed DSH plugin `0.1.1`, extension `ext-v0.1.7` (tag commit `f060a7c074cf1450e4d610aff2906141aebc6718`), CLI `cli-v0.1.11` (tag commit `70b851a3e8408f4994a757e47137ef515bd82901`); CLI archive SHA-256 `041785147342a704fd576470e63307880043a15ad52e0553f12e6dcf360ccf74`; extension artifact SHA-256 `07de2310a6d4c218e017f788f30acbfe88564b0f466e96528d7411fd5bac9ac9`; Sumika does not copy source, browser data, screenshots or logged-in profiles |
| Sumika `plugins/dsh-browser-policy` | DSH `tools/pre-execute` policy adapter and `browser_request_help` bridge | MIT Sumika-authored adapter; uses only public DSH/BrowserSkill interfaces, copies no upstream source, page data or browser assets |
