...
| `web-portals` | 已实现 | 场景壳 dock「门户」（仅桌面版） | [desktop shell](architecture/desktop-shell.md) | [portal commands](../src-tauri/src/main.rs)、[portal tests](../src-tauri/src/main.rs)、[UI smoke](../frontend/tests/smoke.spec.js) | 当前形态：每站独立 WebviewWindow 与持久登录存储（`.sumika-desktop/portals/`）；原始站点无 persona 注入，与 BrowserSkill 网页 Route 完全隔离；预览版无此入口。下一形态（用户已确认方向、未实施）：主窗口内嵌子 webview（需 Tauri unstable）+ composer 站点切换图标，见 desktop-shell.md |
...
| `web-portals` | `PLATFORM-001`, `UX-002` |
