...
| `dsh-agent-runtime` | 部分实现 | Agent | [DSH Agent](integrations/dsh-agent.md) | [DSH adapter](../backend/src/sumika_core/agent/adapters/dsh/runtime.py)、[server tests](../backend/tests/test_agent_runtime.py)、[Playwright smoke](../frontend/tests/smoke.spec.js)、[setup helper](../tools/setup-dsh.ps1) | Provider 档案、Plan/执行 command plane、事件桥、会话快照、停止回合、模型目录/切换、Workspace 登记、可恢复会话分支、Skills/Subagents 查询、审批/问题交互、ToolEventView 安全投影、diff 文件摘要、原始会话 ZIP 导出、session/queue 编辑、会话搜索/重命名、受控图片附件、User Preset 复制/打开/受控删除、只读 `agent.diagnostics` 能力探针和基于会话历史的 MCP 已观察工具清单已具备；下一步完成 Preset 凭据注入、mount validation 和显式 MCP 启用流程，固定版仍无独立 MCP 目录、artifact 管理或 rollback RPC |
