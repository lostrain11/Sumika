...
| `agent-observability` | 部分实现 | `GET /api/agent/observability`；`python tools/aggregate_agent_day.py`；`python tools/evaluate_models.py`；`python tools/capture_model_evaluations.py --opt-in`；`python tools/agent_daily_acceptance.py --real-session`（维护工具） | [Agent observability](architecture/agent-observability.md) | [observability sink](../backend/src/sumika_core/observability.py)、[model evaluator](../backend/src/sumika_core/model_evaluation.py)、[evaluation capture](../backend/src/sumika_core/model_evaluation.py)、[capture CLI](../tools/capture_model_evaluations.py)、[fixed task set](../tools/fixtures/model-evaluation-v1.json)、[evaluation tests](../backend/tests/test_model_evaluation.py)、[capture tests](../tools/test_capture_model_evaluations.py)、[acceptance RPC tests](../backend/tests/test_agent_server.py)、[daily report tests](../tools/test_agent_daily_acceptance.py)、[server tests](../backend/tests/test_observability_server.py) | 累积真实隔离评测样本、固定 manifest 并审查诊断推荐；保持安装/启用/路由改变需批准 |
