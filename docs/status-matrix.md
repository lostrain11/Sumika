...
| `web-chat-runtime` | 部分实现 | Modules > 实现方式、Developer > 网页连接 | [Web Chat](integrations/browser-runtime.md#网页聊天档案web-chat) | [web-chat adapter](../backend/src/sumika_core/browser/web_chat.py)、[runtime tests](../backend/tests/test_web_chat.py)、[route/consultation tests](../backend/tests/test_dynamic_route_supervisor.py)、[RPC tests](../backend/tests/test_web_chat_server.py)、[Playwright smoke](../frontend/tests/smoke.spec.js)；DeepSeek、ChatGPT、智谱、Qwen、Kimi、豆包模板已登记，五成员咨询按 `3 + 2` 运行；2026-09-02 已完成智谱与 Kimi 隔离 Profile 的人工登录、页面检查、长期普通文本授权和可路由验证 | DeepSeek、ChatGPT、Qwen、豆包等网页 Profile 仍需逐站完成真实登录、页面检查、长期普通文本授权和人工接管验证；网页额度保持 `unknown`，不静默路由 |
...
| `web-chat-runtime` | `CAPABILITY-001`, `BROWSER-001`, `BROWSER-002`, `MODEL-002` |
