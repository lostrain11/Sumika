...
本轮H00/H01/H02增量：交接就绪、滚动阶段、可信原生确认、中立Harness契约、受管DSH身份、事件来源绑定及单一冻结DSH发行描述均已接入并完成相称回归（[字段及验证](architecture/task-planning.md)、[发行描述](../dsh-release/README.md)）。H02发行锁定和安装身份防护已完成；候选0.1.5-rc.1因鉴权与协议变更被描述为`blocked`，候选协议迁移及完整运行验收仍未完成。受控模型费用网关、安全续跑和安全合入仍分别属于H03–H05，不能提前标记完成。

H01增量：work/quality预算确认、定时授权、Agent权限回复、问答/Plan Review、MCP配置apply及旧workspace三项写入口已使用[可信原生确认](architecture/host-confirmation.md)，普通HTTP正文不能授权；真实原生smoke已有阶段证据。其余权限入口审计及中立Harness绑定尚未完成，整体状态保持部分实现；不能当作H05安全合入完成。

H01 Skill增量：注册批准/撤销四别名已补可信确认，临时Skill真实HTTP及原生窗口边界通过。retry尚缺可验证未发送事实，中立契约与实例绑定仍待实现，不将注册权限保护当真实Harness运行完成。

H01 retry增量：旧重放入口已要求可信调用者并在缺少恢复证据时固定阻断，提供只读预检和界面提示；不将安全停用标为安全续跑。H03/H04操作级证据未实现，中立契约及实例绑定仍待继续。

H01身份增量：独立SDK RuntimeBinding/ExternalSessionRef、不可热改的registry绑定及只读状态已实现并通过独立wheel测试；受管DSH真实监听、祖先链、启动时间、profile和Job生命周期核验通过，Agent工作/尝试持久绑定及原来源事件核对已接。DSH重启使旧适配器失效，不宣称活动任务热迁移或完整Harness替换完成。

H01 Core启动增量：已通过私有引导HMAC挑战及自有PID认证首次/重启就绪，真实隔离Core重启验证通过。该身份属于Core，不证明DSH稳定profile、发行组合或启动实例，Harness绑定未完成状态保持。
...
| `agent-runtime-portability` | 部分实现 | Agent / Developer / 受管DSH引导 | [Agent Runtime](architecture/agent-runtime.md)、[可信宿主](architecture/host-confirmation.md) | [发行描述](../dsh-release/README.md)、[发行校验](../backend/tests/test_dsh_release.py)、[contracts](../backend/src/sumika_core/agent/contracts.py)、[registry](../backend/src/sumika_core/agent/registry.py)、[中立契约](../packages/quality-routing/src/quality_routing/harness.py)、[实例测试](../backend/src/sumika_core/agent/managed_identity.py)、[权限测试](../backend/tests/test_host_authorization.py) | H02已完成发行锁定、防护和候选阻断修复；当前日用树仍未与冻结锁匹配，候选0.1.5-rc.1的协议迁移及完整会话/工具/事件验收待做；H03受限模型通道、H04显式安全接续待做 |
