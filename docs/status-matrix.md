...
| `desktop-automation` | 部分实现 | Developer > capability catalog；DSH 可选 `desktop_app_*` bridge | [desktop automation](architecture/desktop-automation.md) | [contracts](../backend/src/sumika_core/desktop_automation/contracts.py)、[runtime](../backend/src/sumika_core/desktop_automation/runtime.py)、[adapter tests](../backend/tests/test_desktop_automation.py)、[DSH bridge](../plugins/dsh-desktop-automation/README.md)、[policy tests](../plugins/dsh-desktop-automation/test/policy.test.mjs) | 在隔离环境接入并验证真实 ZCode CDP/UIA runner；继续保持应用登记、租约、审批、敏感输入隔离和前台接管默认关闭 |
