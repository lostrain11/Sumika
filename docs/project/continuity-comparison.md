...
后续精简（R-111）：P4 停止重复采集助手正文和工具标记，跳过没有新增有效事件的存储调用；已有数据不删除。本次进一步阅读 planning-with-files 的 Skill：它约定 task_plan/findings/progress 文件和独立计划目录，声明多种 Hook。直接替换需要处理现有记录迁移与 DSH 的 Hook 实际支持范围，不能仅凭“有现成 Skill”判断能够无缝替换。候选仍应比较，但本次不叠加第二套规划状态。
