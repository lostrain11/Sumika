...
新版本从 [Sumika 新版本项目需求](next-version.md) 开始：基于最新已发布 DSH（包含 RC），先使用原生 Web 完成日用开发平替，新增能力尽量融入 DSH 并保持独立。下文及原需求库保留为旧版记录与参考，不自动全部纳入新版本首期。
...
2. 阅读 [需求总表](catalog.md)、[需求基线](baseline.md)及[内置浏览器正式需求](embedded-browser.md)，了解当前边界和历史取代关系。
...
- 不写入 API Key、Token、Cookie、密码、完整私人聊天、Session ID 或个人绝对路径。允许为追溯需求保存必要的脱敏短摘录、消息UTC时间、角色和定位短语，不复制私人日志。
...
4. 是否有可复现的测试和回滚路径？
5. 凭据、第三方插件、登录或付费动作是否已有覆盖本任务的授权？仅缺失或发生实质变化时补充确认，不要求用户重复授权。
