...
需求汇总见[总表](catalog.md)，消息证据见[原话出处](original-excerpts.md)。`MODEL-012` 至 `MODEL-021`、`TASK-002`、`WORK-001`、`AUTH-001`、`BENEFIT-001` 至 `BENEFIT-003`、`PLUGIN-002`与`PLUGIN-003`承接当前计划。`MODEL-021`与`AUTH-001`的来源是本轮用户显式计划 `visible-conversation-context:2026-09-09`，不伪造消息ID或时间。
...
- `MODEL-021`细化自动/手动绑定：主模型自动模式选质量证据最强的可用候选；角色自动模式只在合格免费候选中优先选择。固定候选失效时提示，不静默替换；修改默认绑定只影响后续请求。
- `AUTH-001`细化付费边界：免费简单任务可直达；付费简单任务、复杂任务及付费规划、验证、咨询、重试、角色附言均须先有有效确认。复杂免费任务也展示范围和零费用预算后等待确认。
...
| `MODEL-021` | 角色与工作模型独立自动/手动选择；主模型质量优先，角色优先合格免费候选。 |
| `AUTH-001` | 所有付费模型用途受预算/授权约束；免费简单任务直达，复杂任务确认后开始。 |
