...
class QualityHttpTests(unittest.TestCase):
    ...
    def test_real_http_role_tool_quote_confirmation_and_delivery(self):
        ...
        ordinary = self.rpc("chat.send", {"character_id": "sumika", "session_id": "default", "messages": [{"role": "user", "content": "hello"}]})
        self.assertEqual(ordinary["message"]["content"], "42")
        self.assertEqual(len(self.calls), 1)
        planned = self.rpc("chat.send", {"character_id": "sumika", "session_id": "default", "messages": [{"role": "user", "content": "Plan the calculation"}]})
        self.assertEqual(planned["task"]["status"], "awaiting-confirmation")
        self.assertEqual(len(self.calls), 3)
        self.assertIsNotNone(planned["task"]["budget"]["quote"]["high_cny"])
        candidate = next(item for item in self.app.quality.engine.candidates() if item.candidate_id == self.candidate)
        self.assertEqual(candidate.estimate(1000000, 1000000), 3)
        task_scope = {**scope, "task_id": planned["task"]["task_id"]}
        self.rpc("quality.task.confirm", {**task_scope, "revision": 1})
        deadline = time.monotonic() + 5
        while time.monotonic() < deadline:
            result = self.rpc("quality.task.get", task_scope)
        ...
        self.assertIsNone(result["results"]["answer"]["cash_cny"])
        self.assertNotIn("hello", self.calls[2]["messages"][0]["content"])
