...
from quality_routing.contracts import amount, count, identifier
from quality_routing.costs import cost_order
...
class SelectionEvidenceStore:
    """Only typed host calls write this bounded, metadata-only store, not RPC."""

    def __init__(self, storage: Any, lock: Any = None) -> None:
...
def resolve_binding(purpose: str, mode: str, fixed_id: str | None, pool: Sequence[str],
    ...
    """Rank exact identities; fixed quality means lead, price only chooses the role.

    Priors break measured-score ties only. Neither record type populates Candidate.quality.
    """
    ...
    for candidate_id in sorted(set(pool)):
        ...
        ranked.append(((*cost_order(quote), candidate_id) if purpose == "role" else quality_key, candidate_id))
    if ranked:
        result.update(candidate_id=min(ranked)[1], reason="highest-qualified-score" if purpose == "leader" else "cheapest-qualified-role")
        if purpose == "leader":
            winning_key, winning_id = min(ranked)
            result["preference_broke_tie"] = winning_id == fixed_id and sum(key[:3] == winning_key[:3] for key, _ in ranked) > 1
    return result
